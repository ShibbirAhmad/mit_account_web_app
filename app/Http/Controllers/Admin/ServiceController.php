    ...
    public function storeClientPayment(Request $request)
    {
        ...
        try {
            ...
            if ($package->is_monthly == 1) {
                ...
                $bills = ServicePackageBill::where('service_package_id', $package->id)->where('status', 0)->orderBy('id', 'asc')->get();
            ...
            if ($data['is_regular'] == 0) {
                ...
                SmsService::servicePaymentConfirmationMessage($request->amount ,$client, $package);
            }
            DB::commit();
            return response()->json([
                'status' => 'OK',

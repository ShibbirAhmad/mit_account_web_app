...
class OrderController extends Controller
{
     ...
     // return $request->all();
        ...
        DB::transaction(function() use($request){
            ...
            if($order->paid>0){
                ...
                $credit->comment ="Order Paid Amount '$order->paid' Paid by '$balance->name'. Order Invoice Number '$order->invoice_no";
   ...
   public function update(Request $request, $id){
    ...
    //  return $request->all();
        ...
        DB::transaction(function() use($order,$request){
            ...
            $order->save();
            $balance=Balance::where('department','mohasagor.com')->where('name',$request->credit_in)->first();
    ...
    public function shipment($id){
        ...
        try{
         if(!empty($order->courier_id) && !empty($order->memo_no)){
            ...
            $order->save();
            SmsService::sendShipmentMessage($order);
            DB::commit();
            return response()->json([
                'status'=>'SUCCESS',
    ...
    public function return($id){
        ...
        DB::transaction(function() use($order){
           ...
           if(count($details) > 0){
            ...
            SmsService::sendReturnMessage(session()->get('admin')['name'],$order->invoice_no,$return_amount);
            $due=CustomerDue::where('memo_no',$order->invoice_no)->first();
            if($due){
                $due->status=1;
                $due->save();
             }
        ...
        if(!empty($request->start_date) && empty($request->end_date)){
            ...
            if($request->status!='all'){
                $orders=Order::whereDate('created_at','=',$request->start_date)
                              ...
                              ->with(['customer:id,name,address','createAdmin:id,name','courier','reseller:id,username','orderItem.product.productImage'])
                              ->paginate($paginate);
            }else{
                $orders=Order::whereDate('created_at','=',$request->start_date)
                              -> with(['customer:id,name,address','createAdmin:id,name','courier','reseller:id,username','orderItem.product.productImage'])
    ...
    public function OrderStatusWise(Request $request){
      ...
      //  return $request->all();
        ...
        $orders=Order::orderBy('id','DESC')->where('status',$request->status)->with(['customer:id,name,address','createAdmin:id,name','courier'])->paginate($paginate);
   ...
   if(strlen($search)==11){
    ...
    $orders = Order::where('cutomer_phone','like', '%' . $search . '%')
                    ->where('status',$request->status)
                    ->orderBy('id', 'DESC')
                    ->with(['customer:id,name,address','createAdmin:id,name','courier','reseller:id,username'])
                    ->paginate(30);
    ...
    {
        $order = Order::where('id', $id)->with(['customer:id,name,address',
            'courier', 'city', 'sub_city', 'approvedBy',
            'orderBarcode', 'resellerOrderDetails',
            'orderPayment', 'createAdmin:id,name',
            'pendingBy', 'shipmentBy',
            'deliveredBy', 'returnBy','cancelBy'
        ])
            ->first();
    ...
    public function exportOrderForPaperFly($id){
        ...
        foreach ($orders as $k => $line) {
            ...
            fputcsv($file, array(
                ...
                'Customer Phone' => $line->cutomer_phone
            ));
        }
        fclose($file);
        return Response::download('./' . $filename);
    ...
    public function exportOrderForRedx($id){
        ...
        foreach ($orders as $k => $line) {
            ...
            fputcsv($file, array(
                ...
                'Division' => "Division",
                'Price' => $total,
                'Instruction' => $line->comment ?? 'None',
                'Product Selling Price' => $total,
                'Seller Name' => 'mohasagor.com',

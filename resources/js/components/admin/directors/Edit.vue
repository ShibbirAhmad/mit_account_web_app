...
export default {
  ...
  methods: {
     ...
     getDirector(){
      ...
      .then(resp => {
        if (resp.data.status="OK") {
            let director = resp.data.director ;
            this.form.name = director.name ;
            this.form.share_value = director.share_value ;
            this.form.share_qty = director.share_qty ;
            this.form.total = director.total_amount ;
            this.form.email = director.email ? director.email : '' ;
            this.form.phone  = director.phone ;
            this.form.address  = director.address ;
        }
      })
    },
    ...
    directorUpdate() {
      this.form
        .post("/api/director/update/info/"+this.$route.params.id)
        .then((resp) => {
          console.log(resp);
          if (resp.data.status == "OK") {

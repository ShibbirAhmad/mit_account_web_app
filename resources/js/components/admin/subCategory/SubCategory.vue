  ...
  <div>
    ...
    <div class="content-wrapper">
      <section class="content-header">
        <h1>
          <router-link :to="{ name: 'subcategoryAdd' }" class="btn btn-primary"
            ><i class="fa fa-plus"></i
          ></router-link>
          <router-link :to="{ name: 'category' }" class="btn btn-success"
            >category</router-link
          >
          <router-link :to="{ name: 'subSubCategory' }" class="btn btn-primary"
            >sub sub category</router-link
          >
        </h1>
        <ol class="breadcrumb">
          <li>
              ...
              <div class="box box-primary">
                  ...
                  <table class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <td>
                          <span v-if="subCategory.status == 1" class="badge"
                            >active</span
                          >
                        ...
                        <td>
                          <span class="badge">
                            {{ subCategory.show_homepage == 1 ? "yes" : "NO" }}
                          </span>
                        </td>
                        <td>
                          <span class="badge">
                ...
                <div class="box-footer">
                  <div class="row">
                    ...
                    >
                      <p>
                        Showing <strong>{{ subCategories.from }}</strong> to
...
export default {
  ...
  methods: {
    ...
    active(subCategory) {
      axios
        .get("/active/subCategory/" + subCategory.id)
        .then((resp) => {
          console.log(resp);
          if (resp.data.status == "SUCCESS") {
            this.subCategory();
    ...
    deActive(subCategory) {
      axios
        .get("/deActive/subCategory/" + subCategory.id)
        .then((resp) => {
          console.log(resp);
          if (resp.data.status == "SUCCESS") {
            this.subCategory();
    ...
    searchResult() {
      if (this.search.length > 1) {
        this.loading = true;
        axios
          .get("/search/subCategory/" + this.search)
          .then((resp) => {
            if (resp.data.status == "SUCCESS") {
              this.loading = false;
              this.subCategories = resp.data.subCategories;
    ...
    getPagination(page = 1) {
      this.loading = true;
      axios.get("/list/subCategory?page=" + page).then((response) => {

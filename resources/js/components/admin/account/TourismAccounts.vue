<template>
  <div>
        ...
        <div class="custom-box">
          <div class="custom-box-body">
            <h4
              ...
              @click="displayTodayTourismCreditAccountDetails(balance)"
              :key="c_index"
            >
              In {{ balance.name }} :
              <strong>{{ parseInt(todayTrourismCreditBalance(balance)) }}</strong>
            </h4>
            <h4>
              In Total : <strong>{{ todayTourismTotalCredit() }} </strong>
            </h4>
          </div>
      ...
      <div class="col-lg-4">
        <div class="custom-box">
          <div class="custom-box-body">
            <h4
              v-for="(balance, c_index) in balance"
              @click="displayTodayTourismDebitAccountDetails(balance)"
              title="click to see details"
              :key="c_index"
            >
              In {{ balance.name }} :
              <strong>{{ parseInt(todayTourismDebitBalance(balance)) }}</strong>
            </h4>

            <h4>
              In Total : <strong>{{ todayTourismTotalDebit() }} </strong>
            </h4>
          </div>
      ...
      <div class="col-lg-4">
        <div class="custom-box">
          <div class="custom-box-body">
            <h4
              v-for="(balance, c_index) in balance"
              @click="displayTourismAccountDetails(balance)"
              title="click to see details"
              :key="c_index"
            >
              In {{ balance.name }} :
              <strong>{{ parseInt(debitTourismCreditSubstraction(balance)) }}</strong>
            </h4>
            <h4>
              In Total <strong> {{ totalTourismBalance() }} </strong>
            </h4>
          </div>
    ...
    <modal name="account_today_tourism_debit_details_modal" :width="800" :height="400">
    ...
    <!-- start today credit payment store modal  -->
    <modal name="account_today_tourism_credit_details_modal" :width="800" :height="400">
      <div class="card" style="padding: 20px">
        <div class="card-header text-center">
          <h4 class="heading">{{ account_name }}</h4>
        </div>
...
export default {
  ...
  methods: {
    ...
    displayTodayTourismDebitAccountDetails(balance) {
      ...
      this.$modal.show("account_today_tourism_debit_details_modal");
    },

    displayTodayTourismCreditAccountDetails(balance) {
      this.account_name = balance.name;
      let credit_history = [];
      ...
      this.$modal.show("account_today_tourism_credit_details_modal");
    },

    displayTourismAccountDetails(balance) {
      this.account_name = balance.name;
      let credit_history = [];
      let debit_history = [];
    ...
    todayTrourismCreditBalance(balance) {
      let amount = 0;
    ...
    todayTourismDebitBalance(balance) {
      let amount = 0;
    ...
    debitTourismCreditSubstraction(balance) {
      if (this.balance.length > 0) {
        let debit_amount = 0;
        let credit_amount = 0;
